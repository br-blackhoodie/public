...
# Setup platform configuration if subscription key provided
if [ -n "$AKE_SUBSCRIPTION_KEY" ]; then
    echo -e "${GREEN}→${NC} Preparing platform configuration..."
    mkdir -p "$INSTALL_DIR/ansible"

    # Configuration sync will be performed on first run
    echo -e "${YELLOW}⚠️  Platform sync will be configured on first run${NC}"
fi
